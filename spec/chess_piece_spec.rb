...
RSpec.describe Chess_piece do
  ...
  describe " #get_pawn_moves" do
    it "can go 1 or 2 squares form starting position and eat by diagonal" do
      test = Chess_piece.new(:pawn, :black, [1, 1])
      board[2][0] = Chess_piece.new(:pawn, :white, [2, 0])
    ...
    it "cant go through alied pieces and can eat enemy pawn en passant" do
      test = Chess_piece.new(:pawn, :black, [1, 1])
      board[1][0] = Chess_piece.new(:pawn, :white, [1, 0])
  ...
  describe " #get_king_moves" do
    ...
    it "can handle very specific castling cases" do
      test = Chess_piece.new(:king, :white, [7, 4])
      board[7][0] = Chess_piece.new(:rook, :white, [7, 0])
      board[7][7] = Chess_piece.new(:rook, :white, [7, 7])
      board[6][0] = Chess_piece.new(:rook, :black, [6, 0])
      board[4][3] = Chess_piece.new(:rook, :black, [4, 3])
  ...
  describe "Defender marker" do
    ...
    it "could properly mark chess piece as a king deffender against diagonal queen" do
      test = Chess_piece.new(:king, :white, [0, 0])
      board[1][1] = Chess_piece.new(:pawn, :white, [1, 1])
      board[3][3] = Chess_piece.new(:queen, :black, [3, 3])
      test.mark_bishop_deffender([0, 0], board)
      expect(board[1][1].king_deffender).to eql(true)
    end    
    ...
    it "could properly mark chess piece as a king deffender against forward queen" do
      test = Chess_piece.new(:king, :white, [0, 0])
      board[0][1] = Chess_piece.new(:pawn, :white, [0, 1])
      board[0][5] = Chess_piece.new(:queen, :black, [0, 5])
      test.mark_rook_deffender([0, 0], board)
      expect(board[0][1].king_deffender).to eql(true)      
    end
